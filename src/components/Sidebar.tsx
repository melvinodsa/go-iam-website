...
import { Schemes, Accordion, Column, Flex, Icon, Row, Tag, ToggleButton, NavIcon } from "@once-ui-system/core";
...
const Sidebar: React.FC<SidebarProps> = ({ initialNavigation, ...rest }) => {
  ...
  return (
    <>
      <NavIcon style={{ position: 'fixed', zIndex: 1001 }} left="8" top="16" hide m={{ hide: false }} onClick={toggleSidebar} />
      <Column
        width={sidebarVisible ? layout.sidebar.width : 0}
        minWidth={sidebarVisible ? layout.sidebar.width : 0}
        position="sticky"
        top="64"
        fitHeight
        gap="2"
        ...
        {...rest}
      >
        {hasLoaded && sidebarVisible && <SidebarContent key={pathname} navigation={navigation} pathname={pathname} />}
      </Column></>
  );
};
